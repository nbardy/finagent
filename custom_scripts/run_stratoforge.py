...
def _summary_markdown(payload: dict, *, top_n: int) -> str:
    ...
    lines.extend(["", "## Model Policy", ""])
    lines.append(
        f"- surface_model=`{model_policy.get('surface_model')}` "
        f"fallback_model=`{model_policy.get('fallback_model')}` "
        f"consensus_structural_models=`{model_policy.get('structural_models_used_in_consensus', [])}`"
    )
    lines.append(
        f"- fixed_grid_available=`{model_policy.get('fixed_grid_pricers_available', [])}` "
        f"fixed_grid_active=`{model_policy.get('fixed_grid_pricers_active', [])}`"
    )
    for model_row in model_policy.get("structural_models_ranked_by_fit", []):
        lines.append(
            f"- fit_rank `{model_row['model']}` "
            f"rmse=`{model_row['rmse']}` "
            f"used_in_consensus=`{model_row['used_in_consensus']}`"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Canonical Stratoforge entrypoint: enumerate and score a thesis-driven options universe.",
    )
    ...
    parser.add_argument(
        "--no-calibrate",
        action="store_true",
        help="Skip stochastic calibration and score with BS / market-mid fallback only.",
    )
