...
def _default_output_base(thesis: ThesisSchema) -> Path:
    ...
    return date_dir / f"bates_calibration_benchmark_{thesis.symbol.lower()}_{timestamp}"


def _initial_guess(spot: float, quotes: list[MarketQuote]) -> np.ndarray:
    atm_iv = max(q.market_price for q in quotes) / spot
    guess = np.array([atm_iv**2, atm_iv**2 * 0.75, 2.0, 0.5, -0.5, 0.75, -0.05, 0.12], dtype=float)
    lower, upper = _bounds()
    return np.clip(guess, lower + 1e-8, upper - 1e-8)


def _bounds() -> tuple[np.ndarray, np.ndarray]:
    lower = np.array([0.01, 0.01, 0.1, 0.01, -0.99, 0.0, -0.5, 0.01], dtype=float)
    upper = np.array([10.0, 10.0, 20.0, 3.0, 0.0, 10.0, 0.1, 1.0], dtype=float)
    return lower, upper
...
def _summarize_fit(
    *,
    ...
    params = BatesParams(
        ...
        lam=float(result_x[5]),
        mu_j=float(result_x[6]),
        sigma_j=float(result_x[7]),
    )
    ...
    return {
        ...
        "params": {
            ...
            "lam": round(params.lam, 8),
            "mu_j": round(params.mu_j, 8),
            "sigma_j": round(params.sigma_j, 8),
        },
...
def _render_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# Bates Calibration Solver Benchmark — {payload['thesis_symbol']}",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark Bates calibration solver settings on a thesis-derived quote subset.")
    ...
    for result in results:
        result["speedup_vs_current"] = round(baseline_time / result["wall_time_s"], 4)
        result["rmse_delta_vs_current"] = round(result["rmse"] - baseline_rmse, 6)
    ...
    for result in results:
        print(
            ...
            f"speedup={result['speedup_vs_current']:.4f}x "
            f"nfev={result['nfev']}"
        )
